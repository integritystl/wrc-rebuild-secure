...
function articulate_quiz_embeder_new_db_install()
{
...
}#end function articulate_quiz_embeder_new_db_install

function articulate_quiz_embeder_db_update()
{
...
wp_die("articulate_quiz_embeder_db_update process needed. I came from function articulate_quiz_embeder_db_update() in activate.php in 'Insert or Embed Articulate Content into Wordpress' plugin");
}
...
function articulate_quiz_embeder_install() {
   ...
   {
   articulate_quiz_embeder_new_db_install();
   ...
   else
   {
		...
		if ($version_comp ==0)#if same version
		{
			...
			articulate_quiz_embeder_new_db_install();# New installation process checks all tables are exists or not
		}
		elseif($version_comp < 0)
		{
		 articulate_quiz_embeder_db_update();
		}
		else
		{
...
}#end function articulate_quiz_embeder_install()

# helps: http://oneTarek.com , http://php.net/manual/en/function.version-compare.php
function articulate_quiz_embeder_update_db_check() {
    global $quiz_embeder_db_version;
...
add_action('plugins_loaded', 'articulate_quiz_embeder_update_db_check' );

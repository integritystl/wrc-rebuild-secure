...
class WSAL_Rep_Common {
	 ...
	 *
	 * @deprecated 4.1.3
	 * @see WSAL_Rep_Common::DeleteGlobalSetting()
	 */
	 ...
	 *
	 * @deprecated 4.1.3
	 * @see WSAL_Rep_Common::GetSettingByName()
	 */
	...
	private function _getAlertDetails( $entryId, $alertId, $siteId, $createdOn, $userId = null, $roles = null, $ip = '', $ua = '' ) {
		...
		$out = array(
			...
			'date'       => str_replace(
				'$$$',
				substr( number_format( fmod( (int) $createdOn + $this->_gmt_offset_sec, 1 ), 3 ), 2 ),
				date( $this->_datetimeFormat, (int) $createdOn + $this->_gmt_offset_sec )
			),
			'code'       => $const->name,
	...
	public function GenerateReport( array $filters, $validate = true ) {
		...
		$objects       = ( empty( $filters['objects'] ) ? null : $filters['objects'] );
		$event_types   = ( empty( $filters['event-types'] ) ? null : $filters['event-types'] );
		...
		if ( $objects ) {
			...
			foreach ( $objects as $object ) {
				array_push( $_objects, preg_quote( $object ) );
			}
			$_objects = "'" . implode( ',', $_objects ) . "'";
		}
		...
		if ( $event_types ) {
			...
			foreach ( $event_types as $event_type ) {
				array_push( $_event_types, preg_quote( $event_type ) );
			}
			$_event_types = "'" . implode( ',', $_event_types ) . "'";
		}
		...
		if ( ! empty( $filters['unique_ip'] ) ) {
			...
			$results = $this->wsal->getConnector()->getAdapter( 'Occurrence' )->GetReporting( $_siteId, $_userId, $_roleName, $_alertCode, $_startTimestamp, $_endTimestamp, $_nextDate, $_limit, $_post_types, $_post_statuses, $_objects, $_event_types );
		}
	...
	private function FileGenerator( $data, $filters ) {
		...
		$csvReport = new WSAL_Rep_CsvReportGenerator( $this->wsal->settings()->GetDatetimeFormat( false ) );
		// Report Number and list of unique IP.
		if ( ! empty( $filters['unique_ip'] ) ) {
	...
	final public function custom_wp_mail_from( $original_email_from ) {
		$use_email  = $this->wsal->settings()->get_use_email();
		$email_from = $this->wsal->settings()->GetFromEmail();
	...
	final public function custom_wp_mail_from_name( $original_email_from_name ) {
		$use_email       = $this->wsal->settings()->get_use_email();
		$email_from_name = $this->wsal->settings()->GetDisplayName();
	...
	public function SwitchToArchiveDB() {
		if ( $this->IsArchivingEnabled() ) {
			$archive_type       = $this->GetSettingByName( 'archive-type' );
			$archive_user       = $this->GetSettingByName( 'archive-user' );
			$password           = $this->GetSettingByName( 'archive-password' );
			$archive_name       = $this->GetSettingByName( 'archive-name' );
			$archive_hostname   = $this->GetSettingByName( 'archive-hostname' );
			$archive_baseprefix = $this->GetSettingByName( 'archive-base-prefix' );
			$archive_ssl        = $this->GetSettingByName( 'archive-ssl', false );
			$archive_cc         = $this->GetSettingByName( 'archive-client-certificate', false );
			$archive_ssl_ca     = $this->GetSettingByName( 'archive-ssl-ca', false );
			$archive_ssl_cert   = $this->GetSettingByName( 'archive-ssl-cert', false );
			$archive_ssl_key    = $this->GetSettingByName( 'archive-ssl-key', false );

...
class WSAL_Rep_Views_Main extends WSAL_AbstractView {
	...
	public function Header() {
		...
		wp_enqueue_style( 'wsal-jq-ui-css', $this->_base_url . '/js/jquery.datepick/smoothness.datepick.css', array(), '5.0.0' );
		...
		wp_enqueue_script(
			'wsal-jq-datepick-plugin-js',
			$this->_base_url . '/js/jquery.datepick/jquery.plugin.min.js',
			array( 'jquery' ),
			'1.0.1',
			false
		);
		...
		wp_enqueue_script(
			'wsal-jq-datepick-js',
			$this->_base_url . '/js/jquery.datepick/jquery.datepick.min.js',
			array( 'jquery' ),
			'5.0.0',
			false
		);
					...
					success: function(response) {
						checkStatus = setInterval( function() {
							...
							if (nextDate != 0) {
								...
								jQuery("#ajax-response").html("Email sent.").addClass( 'sent' );
								clearInterval( checkStatus );
							} else {
								if ( ! jQuery( '#ajax-response.sent' ).length ) {
									jQuery("#ajax-response").html("No events to report.");
									clearInterval( checkStatus );
								};
							}
						}, 100);
	...
	public function SavePeriodicReport( $post_data ) {
		if ( isset( $post_data ) ) {
			...
			$this->_plugin->wsalCommonClass->AddGlobalSetting( $optName, $data );
		}
	}
	...
	private function generateStatisticsReport( $filters ) {
		...
		if ( isset( $_POST['include-archive'] ) ) {
			$this->_plugin->wsalCommonClass->AddGlobalSetting( 'include-archive', true );
		} else {
			$result = $this->_plugin->reporting->common->DeleteGlobalSetting( 'wsal-include-archive' );
		}
		?>
		<script type="text/javascript">
	...
	public function Render() {
		...
		if ( ! is_wp_error( $wpsalRepUploadsDir ) && $this->_plugin->reporting->common->CheckDirectory( $wpsalRepUploadsDir ) ) {

...
jQuery(document).ready(function () {
	...
	// watch for main checkbox changes to determin enabled/disabled states.
 ...
 *
 * @param {integer} offset   - User's offset.
 * @param {integer} security - Security nonce.
 */
function terminate_all_sessions( offset, security ) {
		...
		success: function( response ) {
			if ( response.success ) {
				...
				if ( 0 !== response.offset ) {
					progress.append( '<p>' + response.offset + ' ' + script_data.sessionsTerminted + '</p>' );
					terminate_all_sessions( offset, security );
				} else {
...
/**
 * Click hanlder that initiates event data fetching.
 *

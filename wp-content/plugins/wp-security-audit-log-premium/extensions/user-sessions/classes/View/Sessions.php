...
class WSAL_UserSessions_View_Sessions {
	...
	/**
	 * GMT offset to apply to dates being displayed.
	 *
	 * @var int
	 */
	...
	public function __construct( WpSecurityAuditLog $wsal ) {
		...
		// Set the GMT offset.
		$this->gmt_offset = $this->GetGmtOffset();
	...
	public function render() {
		...
		$type_name = $this->wsal->settings()->get_type_username();
		...
		// TODO: sorting needs rebuild here - results is not an array.
		// if ( 'asc' === $order ) {
		...
		$datetime_format = $this->wsal->settings()->GetDatetimeFormat();
										...
										<td colspan="6">
											<table class="wp-list-table widefat fixed users">
												...
												foreach ( $result_sessions as $key => $result ) :
													...
													$session_times_format = str_replace( '.$$$', '', $datetime_format );

													$created    = date( $session_times_format, $result->creation_time + $this->gmt_offset );
													$expiration = date( $session_times_format, $result->expiry_time + $this->gmt_offset );
	...
	protected function sessions_search() {
		...
		if ( ! is_array( $this->all_tracked_sessions ) || empty( $this->all_tracked_sessions ) ) {

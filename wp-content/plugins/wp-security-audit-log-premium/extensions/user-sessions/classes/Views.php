...
class WSAL_UserSessions_Views extends WSAL_AbstractView {
	...
	public function Footer() {
		...
		$total_users = count_users( 'time', $this->_plugin->settings()->get_view_site_id() );
		$total_users = isset( $total_users['total_users'] ) ? $total_users['total_users'] : false;
		...
		$script_data = array(
			...
			'sessionsTerminted' => sprintf( __( 'out of %s users sessions terminated.', 'wp-security-audit-log' ), $total_users ),
		...
		if ( isset( $_GET['terminate'] ) && 'all-sessions' === sanitize_text_field( wp_unslash( $_GET['terminate'] ) ) ) {
			...
			<script type="text/javascript">
				jQuery( document ).ready( function() {
					terminate_all_sessions( 0, "<?php echo esc_html( wp_create_nonce( 'wsal-terminate-all-sessions' ) ); ?>" );
				});
			</script>
			<?php
		} else {
	...
	public function ajax_destroy_user_session() {
		...
		if ( ! empty( $post_array['nonce'] )
			...
			// delete the session from the custom table.
			$adapter = WSAL_UserSessions_Plugin::get_sessions_adapter();
			$sessions   = $adapter->delete_by_session_token( $post_array['token'] );

			// clear the core sessions data in usermeta.
			$user_sessions = WSAL_UserSessions_Plugin::get_user_session_tokens( $user_id );
			if ( ! empty( $user_sessions ) && array_key_exists( $post_array['token'], $user_sessions ) ) {
				unset( $user_sessions[ $post_array['token'] ] );
				update_user_meta( $user_id, 'session_tokens', $user_sessions );
			}

			// fire event 1007 destoryed session and logged out.
	...
	public function terminate_all_sessions() {
		...
		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'wsal-terminate-all-sessions' ) ) {
			// Get offset.
			$users_offset = isset( $_POST['offset'] ) ? (int) sanitize_text_field( wp_unslash( $_POST['offset'] ) ) : false;
			if ( false !== $users_offset ) {
				$users_limit = 50;
				$adapter = WSAL_UserSessions_Plugin::get_sessions_adapter();
				...
				if ( ! empty( $result_users ) ) {
					...
					if ( ! empty( $deleted_user_ids ) ) {
						$adapter->delete_by_user_ids( $deleted_user_ids );
					}

					$response = array(
						'success' => true,
						'offset'  => $users_offset + $users_limit,
						'message' => esc_html__( 'Sessions destroyed!', 'wp-security-audit-log' ),
					);
				} else {
					if ( ! $this->_plugin->IsMultisite() ) {
						// $this->_plugin->usermanagement->common->delete_users_sessions_transient();
					} else {
						// $this->_plugin->usermanagement->common->delete_users_sessions_transient( $this->get_sites() );
					}

					$current_user_id = get_current_user_id();
					delete_user_meta( $current_user_id, 'session_tokens' );

					$adapter->delete_by_user_ids( array( $current_user_id ) );
				...
				$response = array(
					'success' => false,
					'message' => esc_html__( 'Users offset is not set.', 'wp-security-audit-log' ),
				);
				echo wp_json_encode( $response );
			}
		} else {
	...
	public function fetch_session_event_data_chunk() {
		...
		if ( ! empty( $session_chunk ) ) {
			...
			foreach ( $session_chunk as $key => $session_data ) {
				...
				if ( $user_last_alert ) {
					...
					$last_alert['object']     = $user_last_alert->GetMetaValue( 'Object' );
					$last_alert['event_type'] = $user_last_alert->GetMetaValue( 'EventType' );
				} else {
					$last_alert['created_on'] = false;
					$last_alert['event_id']   = false;
					$last_alert['client_ip']  = false;
					$last_alert['object']     = false;
					$last_alert['event_type'] = false;
				}
				$send_data[ $session_data['session'] ] = $last_alert;
			}
		} else {
		...
		// should never reach this point but just incase die here.
		die();
	}
}

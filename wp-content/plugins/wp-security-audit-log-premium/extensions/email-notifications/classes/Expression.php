...
class WSAL_NP_Expression {
	...
	final public function __construct( WSAL_NP_Notifier $notif, array $s1_data, array $s2_data, array $s3_data, $title, $s4_data = array(), $s5_data = array(), $s6_data = array(), $s7_data = array(), $s8_data = array() ) {
		...
		$this->_s7_data = $s7_data;
		$this->_s8_data = $s8_data;
		$this->title    = $title;
	}
	...
	final protected function _evaluateTrigger( array $condition ) {
		...
		$s4 = isset( $condition['select4'] ) ? $this->_s4_data[ $condition['select4'] ] : false;
		$s5 = isset( $condition['select5'] ) ? $this->_s5_data[ $condition['select5'] ] : false;
		$s6 = isset( $condition['select6'] ) ? $this->_s6_data[ $condition['select6'] ] : false;
		$s7 = isset( $condition['select7'] ) ? $this->_s7_data[ $condition['select7'] ] : false;
		$s8 = isset( $condition['select8'] ) ? $this->_s8_data[ $condition['select8'] ] : false;
		$i1 = $condition['input1'];
		return $this->notif->_checkIfConditionMatch( $s1, $s2, $s3, $s4, $s5, $s6, $s7, $s8, $i1, $this->title );
	}
}

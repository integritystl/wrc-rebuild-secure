...
class WSAL_NP_AddNotification extends WSAL_AbstractView {
	...
	public function Header() {
		wp_enqueue_style(
			'wsal-jq-datepick-css',
			$this->_base_url . '/js/jquery.datepick/smoothness.datepick.css',
			array(),
			'5.0.0'
		);
		wp_enqueue_style(
		...
		<script type="text/javascript">
			...
			// Get WP user roles.
			$wp_user_roles = $this->_plugin->wsalCommon->get_wp_user_roles();
		...
		<?php
		wp_enqueue_script(
			'wsal-jq-datetime-pick-plugin-js',
	...
	public function Render() {
		...
		<div class="wrap">
			...
			<script type="text/template" id="scriptEmail">
				...
				<p class="description"><?php esc_html_e( 'Specify the email addresses, WordPress usernames or phone numbers of those who should receive the notification once the trigger is matched. To specify multiple recipients separate them with a comma (,).', 'wpsal-notifications' ); ?></p>

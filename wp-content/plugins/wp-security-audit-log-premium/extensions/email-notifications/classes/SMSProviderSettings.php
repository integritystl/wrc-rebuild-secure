...
class WSAL_NP_SMSProviderSettings {
	...
	public function sms_settings_tab() {
		...
		$settings_tab = add_query_arg( array_merge( $sub_tab_args, array( 'section' => 'settings' ) ), admin_url( 'admin.php' ) );
		$test_tab     = add_query_arg( array_merge( $sub_tab_args, array( 'section' => 'test' ) ), admin_url( 'admin.php' ) );
		?>
	...
	private function settings_tab() {
		...
		?>
		<p class="description"><span class="dashicons dashicons-info"></span> <?php echo wp_kses( $tab_info_msg, $html_tags ); ?></p>
	...
	public function send_sms( $to_number, $message ) {
		if ( $to_number && $message && $this->account_sid && $this->auth_token ) {
			// Include Twilio SDK.
			require_once trailingslashit( WSAL_BASE_DIR ) . 'sdk/twilio-php/Twilio/autoload.php';
